...
async function getSigningKey(kid: string): Promise<string> {
  console.log("this.authzOptions", this.authzOptions)
  for (const authz of this.authzOptions) {
    const client = jwksClient({
      cache: true,
      rateLimit: true,
      jwksRequestsPerMinute: 10, // Default value
      jwksUri: authz.jwksUri,
    });
...
export function makeAuthorizer(
  ...
  const getSignKey = getSigningKey.bind({ authzOptions: authzOptions });
  const authz = authorize.bind({ headers, authzOptions, getSignKey });
...
async function authorize(): Promise<IPayload> {
  ...
  const getSignkey = this.getSignKey
  ...
  const signingKey = await getSignkey(decoded.header.kid);

  for (const data of authzOptions) {
